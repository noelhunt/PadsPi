...
DType::DType(const DType& dt){			// sunos5symtab.cc
	univ = dt.univ;
	pcc = dt.pcc;
	dim = dt.dim;
	over = dt.over;
...
int DType::iscnst() 		{ return ISCNST(pcc); }
...
const char *DType::text(){
	...
	if( iscnst() ) return sf( "const %s", ref()->text() );
	...
	if( f ){
		...
		for( bit = 1; bit; bit <<= 1 ) if( bit & f ){
			...
			if( b&over ) b |= F_TURNOFF;
	    		s.last(FmtName((int)b), (Action)&Phrase::reformat, b);
		}
