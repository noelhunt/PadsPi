...
#include "symtab.h"
#include "core.h"
#include <signal.h>		/* floating point exceptions */
extern "C" {
	double fabs(double);
}
SRCFILE("expr.c")
...
char NULLSTR[4] = {0, 0, 0, 0};

const char *OpName(Op op){
	switch(op){
	case O_CONST:	return "const";
	case O_DEREF:	return "*";
	case O_REF:	return "&";
	case O_INDEX:	return "[]";
	case O_DOT:	return ".";
...
Expr *E_Id(const char *id ) { return new Expr(E_ID,(Op)0,0,0,0,id,0,0); }

Expr *E_Sym(Symbol *s ) { return new Expr(E_SYMNODE,(Op)0,0,0,0,0,0,s); }
...
Expr *E_IConst(long i) { return new Expr(E_LCONST,(Op)0,0,0,i,Token,LONG,0); }

Expr *E_DConst(double d) { return new Expr(E_DCONST,(Op)0,0,0,d,Token,DOUBLE,0); }

const char *Expr::floaterror(){
	return val.floaterror();
}

const char *Expr::under(Expr *e){
	if( e->edisc != E_BINARY || Prec((Op)e->op) < Prec((Op)op) ) return e->text();
	return sf( "(%s)", e->text() );
}

int Expr::disc()	{ return U_EXPR; }

Expr::Expr() {}

Expr::Expr( EDisc d, Op o, Expr* s1, Expr* s2, Cslfd c, const char* s, int t, Symbol *v){
	type.pcc = t;
	edisc = d;
	op = o;
	sub1 = s1;
	...
	trace( "Expr(edisc=%d,op=%s,sub1=%d,sub2=%d,val.lng=%d,id=%s,symbol=%d",
		edisc, OpName(o), sub1, sub2, val.lng, id?id:"", v); 
}

const char *Expr::left(Expr *e){
	switch( e->edisc ){
...
const char *Expr::textunary(){
	trace( "%d.textunary()", this );	OK("textunary");
	switch( op ){
	case O_QCAST:
	case O_QSTRUCT:
	case O_QENUM:
		return sf("(%0.*s%s )%s",val.lng,"*******",OpName((Op)op),sub1->text());
	case O_CAST:
	...
	case O_FABS:
	case O_SPECIAL:
		return sf( "%s(%s)", OpName((Op)op), sub1->text() );
	}
	if( sub1->edisc==E_BINARY && Prec((Op)sub1->op)>=Prec(O_MULT) )
		return sf( "%s(%s)", OpName((Op)op), sub1->text() );
	return sf( "%s%s", OpName((Op)op), sub1->text() );
}

const char *Expr::textbinary(){
	...
	case O_DOT:
	case O_ARROW:
		return sf("%s%s%s", left(sub1), OpName((Op)op), sub2->text());
	}
	if( sub1->edisc==E_BINARY && Prec((Op)sub1->op) == Prec((Op)op) )
		return sf("%s%s%s", sub1->text(), OpName((Op)op), under(sub2));
	return sf("%s%s%s", under(sub1), OpName((Op)op), under(sub2));
}

const char *Expr::text(){
	trace( "%d.text()", this );	OK("text");
	switch( edisc ){
	case E_DCONST:	if( !id || !*id ) return sf( "%.15g", val.dbl );
...
const char *Expr::ascii(Frame *frame, int limit ){
	static char buf[256];
	char *raw, *fail = (char *) "fail";	// fail is unique
	Bls esc( "\"" );
	int i;
...
const char *Expr::utypeformat(Frame *frame, Bls &build){
	Var *m, g(0,0,0,(UDisc)0,"$pi.Expr::utypeformat");
	int i = 0;
...
const char *Expr::evaltextcomma(Frame *frame, Bls &build){
...
const char *Expr::evaltext(Frame *frame, Bls &build){
	trace( "%d.evaltext(%d,%d)", this, frame, &build ); OK("evaltext");
	evalerr = 0;
	doevaltext(frame, build);
	if( evalerr ) setspy(0);
	if( spy ) spy->b = build;	/* NOTE */
	return build.text;
}

const char *Expr::doevaltext(Frame *frame, Bls &build){
	const char *error, *t;
...
const char *Expr::getval( Frame *frame ){		/* addr already fixed */
...
const char *Expr::invalidoperands(const char *more){
	trace( "%d.invalidoperands()", this );	OK("invalidoperands");
	const char *colon = more ? ": " : "";
	if( !more ) more = "";
	return sf( "invalid operand(s) of %s%s%s", OpName((Op)op), colon, more );
}

const char *Expr::eval(Frame *frame){
...
const char *Expr::enumid(Frame *frame){
...
const char *Expr::evalindex(Frame *frame, Expr *ap, long i){
	long size = 0;
...
const char *Expr::evaldotarrow(Frame *frame){
	DType t;
...
const char *Expr::evalcall(Frame *frame){
	const char *error = 0;
	...
	while( sub2->actual((int)nargs) )
		if( !(formal = func->argument((int)nargs)) )
			return "too many args in function call";
		else {
			argwords += (formal->type.size_of()+3)/4;   /* vax & 32 */
			++nargs;
		}
	if( func->argument((int)nargs) )
	...
	called->ap = core->apforcall((int)argwords*4);
	if( !called->ap )
		{ error = "function calling is broken"; goto Restore; }
	for( i = 1; i <= nargs; ++i ){
		Expr *e;
		e = E_Binary(E_Sym(func->argument((int)i)), O_ASSIGN, sub2->actual((int)i));
	...
	if( error = core->docall(func->range.lo, (int)argwords) )
		goto Restore;
...
const char *Expr::evalenv(Frame *frame){
	...
	for( ; env; env = env->caller() )
		if( !strcmp(env->func->_text,sub2->text()) )
			break;
	if( !env ){
		Func *funcp = (Func*) frame->core->symtab()->
				idtosym(U_FUNC,sub2->text());
		if ( funcp ) {
			sta = new Frame(frame->core);
			sta->func = funcp;
			env = sta;
		}
		else
			return sf( "not found: %s()", sub2->text() );
	}
	if( error = sub1->eval(env) ) goto out;
	type = sub1->type;
	val = sub1->val;
	addr = sub1->addr;
out:
...
static Expr *exprptr;
static double *fp1, *fp2;
static const char *fpe;

static void fpehandler(int)	{ exprptr->catchfpe(); }
...
const char *Expr::evalflop(){		/* VAX host */
	const char *error;
	...
	fpe = 0;
	exprptr = this;
	signal(SIGFPE, (SIG_TYP)fpehandler);
	switch( op ){
...
const char *Expr::evalbinary(Frame *frame){
	const char *error;
	long size;
...
const char *Expr::evalunary(Frame *frame){
	const char *error;
...
const char *Expr::evalassign(Frame *frame){
	const char *error;
