...
void DwarfSymTab::dosymtab(Var *glb, DwarfSource *src){
	...
	for (i = 0; i < e->nsymtab; i++) {
		...
		case STT_OBJECT:
			...
			name = s.name;
			/*
			 * Sun Studio Compiler specific:
			 *
			 * File statics are encoded as <ENCODE>varname, local
			 * statics as <ENCODE>funcname.varname, where <ENCODE>
			 * is a string of the form '.X[AB][A$][a-zA-Z0-9]{12}.'
			 * in semi-regexp notation.
			 */
			if (name[0] == '.' && name[16] == '.'){
				char c = name[17];
				name[17] = 0;
				if( !fsyms->search(name) ){
					/* we have a static from an external
					 * source -- ignore for now
					 */
				}
				break;
			}
