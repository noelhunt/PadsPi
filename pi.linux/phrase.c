...
int Phrase::disc()	{ return U_PHRASE; }
...
const char *Phrase::enchiridion(long l){
...
void Phrase::increfcast(long i){
	...
	Expr *e = E_Unary((Op)expr->op, expr->sub1);
...
void Phrase::soretycast(long i, short qop){
	...
	if( !iscast() )
		expr = E_Unary(O_QCAST, expr);
	Expr *e = E_Unary((Op)qop, expr->sub1);
	e->val.lng = i + expr->val.lng;
	derive(e);
}
