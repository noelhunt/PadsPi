...
int Process::disc()	{ return U_PROCESS; }

void Process::pop(long isinstr){
	trace("%d.pop()", this); VOK;
	if( core->behavs() == ACTIVE ){
		insert(ERRORKEY, "pop callstack: process not stopped");
		return;
	}
	journal()->insert("pop");
	const char *err = core->popcallstack();
	habeascorpus(isinstr ? INST_STEPPED : STMT_STEPPED, changes());
	insert(ERRORKEY, err);
}
...
const char *Process::enchiridion(long l){
	switch(l) {
		case HELP_OVERVIEW:	return "Process Window";
		case HELP_MENU:		return "Process Menu Bar";
		case HELP_KEY:		return "Process Keyboard";
		case HELP_LMENU:	return "Process Line Menus";
		default:		return 0;
	}
}

Process::Process(Process *sib, const char *p, const char *s, const char *c){
	extern char *SRCPATH;
...
void Process::openpad(){
	...
	if( !pad ){
		...
		pad->options(TRUNCATE|NO_TILDE|ACCEPT_KBD|USERCLOSE);
		banner();
...
void Process::merge(){
	...
	if( !callstk ){
		...
		insert(ERRORKEY, "cannot obtain callstack (%s)", core->behavsnam());
		return;
	}
	if( !old ) return;
...
void Process::mergeback(long){		/* arg never used - should go away */
	...
	merge();
	pad->lines(padlines = ERRORKEY);
	if( callstk ) pad->lines(padlines = (int)callstk->size+ERRORKEY);
...
void Process::linereq(int l, Attrib a){
...
void Process::insert(long l, const char *fmt, ...){
	va_list ap;
	va_start(ap, fmt);
...
void Process::stepinto(){
	const char *error = 0;
	class Stmt *stmt;
	time_t t0 = time((time_t *)0), timeout = 10, changed = 0;
	...
	while( !error && changed==0 ){
		...
		error = core->step();
		if( !error && time((time_t *)0) > t0+timeout )
...
void Process::stepover(long lo, long hi){
	...
	stoprequest = 0;
	const char *error = core->step(lo, hi);
	insert(ERRORKEY, error);
	if( error ){
		docycle();
		return;
...
void Process::habeascorpus(Behavs b, long c){
	...
	trace( "%d.habeascorpus(b.%d, c.%d)", this, b, c );	VOK;
	t->clear();
...
void Process::error(const char *err){
	insert(ERRORKEY, err);
}
