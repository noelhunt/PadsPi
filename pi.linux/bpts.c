...
int BptReq::disc()	{ return U_BPTREQ; }

char *BptReq::setline(Process *p){
	Source *s;
	trace("%d.set(%d)", this, p); OK("BptReq::setline");
	for( s = p->symtab()->root(); s; s = (Source*)s->rsib )
		if( !strcmp(s->text(), file)
		 || !strcmp(basename(s->text()), file) )
			break;
	if( !s )
		return sf("source file not found: %s", file);
	Func *f = s->funcafter((int)line, 0);
	if (!f)
		return sf("function not found: line: %d", line);
	Stmt *stmt = s->stmtafter(f, (int)line);
	if( !stmt )
...
BptReq::BptReq(char *f, char *curly, char *e){
	...
	file = 0;
	func = f;
	for( i = 0; *curly; ++curly ) switch(*curly){
		case '{': i |= 1; break;
		case '}': i |= 2; break;
	}
	switch (i) {
...
const char *Trap::liftorlay(LiftLay lol, Core *core){
...
Bpts::Bpts(Core *c){
	...
	pad->menu(m.index("bpts"));
	banner();
}

const char *Bpts::enchiridion(long l){
	switch(l) {
		case HELP_OVERVIEW:	return "Breakpoint Window";
		case HELP_MENU:		return "Breakpoint Menu Bar";
		case HELP_LMENU:	return "Breakpoint Line Menus";
		default:		return 0;
	}
}
...
void Bpts::select(Trap *t){
	...
	if( t->stmt ){
		...
		if( t->stmt->condition && t->stmt->condition!=Q_BPT )	/* ? */
			buf.af( " %s", t->stmt->condition->text() );
	}
	if( t->error ){
		a |= SELECTLINE;
		buf.af( "%s", t->error );
