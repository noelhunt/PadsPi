	...
	while (--argc > 0 && (++argv)[0][0]=='-')
		...
		case 'A':		/* for acro */
			for(cc='A';cc<='Z';cc++)
				table[cc] = cc;
			continue;
		case 'o':		/*to put hits to file*/
			listn = (++argv)[0];
			argc--;
			list++;
			if((fl=fopen(listn,"w"))== NULL){
				fprintf(stderr,"diction: can't open file %s\n",
					listn);
				exit(2);
			}
			continue;
...
out:
	...
	if(fopen(CATCH,"r") != NULL){
		if((mine=fopen(CATCH,"a"))==NULL)mflg=0;
		else mflg = 1;
	}
#else
	mflg = 0;
...
init:	if ((s->inp) != 0) {
		*rear++ = s->nst;
		if (rear >= &queue[QSIZE - 1]) overflo("queue1");
	}
	if ((s = s->link) != 0) {
		goto init;
	}
			...
			*rear = (q = s->nst);
			if (front < rear)
				if (rear >= &queue[QSIZE-1])
					if (front == queue) overflo("queue2");
					else rear = queue;
				else rear++;
			else
				if (++rear == front) overflo("queue3");
			state = s->fail;
		floop:	if (state == 0){ state = w;bstart=1;}
			if (state->inp == c) {
			qloop:	q->fail = state->nst;
				if ((state->nst)->out != 0 && q->out == 0){
					q->out = (state->nst)->out;
					q->code = (state->nst)->code;
				}
				if((q=q->link) != 0)goto qloop;
/*				do {
					q->fail = state->nst;
					if((state->nst)->out != 0)q->out=(state->nst)->out;
				}while((q=q->link) != 0);*/
			}
...
void outc(char *addr, char *file, char code){
	...
	while(nlp < addr){
		...
		if(inside && caps){
			...
			if(inside && list)putc(table[*nlp],fl);
			if(!caps && *nlp == '\n')*nlp = ' ';
			if(*nlp == ' ' && oct==0);
